...
char *add_new_file(const char *base_name, const char *new_extension) {
    size_t len = strlen(base_name) + strlen(new_extension) + 2; // 1 for dot/extension, 1 for null terminator
    char *new_file = (char *)handle_malloc(len);
    strcpy(new_file, base_name); // Copy base filename
    strcat(new_file, new_extension); // Append the new extension
    return new_file;
}
...
char *short_to_base64(unsigned short value) {
    ...
    char *encoded = (char *)handle_malloc(3); // 2 chars + null terminator
    encoded[0] = base64_chars[(value >> 6) & 0x3F]; // Get the first 6 bits
    encoded[1] = base64_chars[value & 0x3F];        // Get the last 6 bits
    encoded[2] = '\0';
    return encoded;
}
